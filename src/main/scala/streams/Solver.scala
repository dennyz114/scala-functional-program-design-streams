...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    val list: List[(Block, List[Move])] =
      for (newBlock, newMove) <- b.legalNeighbors
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter((neighbor, _) => !explored.contains(neighbor))
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] =
    val a: LazyList[(Block, List[Move])] = for
      (block, moves) <- initial
      newNeighborAndMoves <- newNeighborsOnly(neighborsWithHistory(block, moves), explored)
    yield
      newNeighborAndMoves

    initial #::: from(a, explored ++ a.map(_._1))
